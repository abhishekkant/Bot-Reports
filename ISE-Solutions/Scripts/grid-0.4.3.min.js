...
﻿gj.grid.private = {
    ...
    CellRenderer: function ($grid, $cell, column, record, id, mode) {
        ...
        if ("checkbox" === column.type) {
            if ("create" === mode) {
                $wrapper.append($("<input />").attr("type", "checkbox").val(record[column.field]).click(function (e) {
                    e.stopPropagation();
                    $grid.setSelected(this.value);
                }));
            } else {
                $wrapper.find("input[type='checkbox']").val(record[column.field]).prop("checked", false);
            }
        }
       ...
       else if ("image" === column.type) {
          // alert(column.type + "-" + mode)
            if ("create" === mode) {
               // alert('dfdf');
                $wrapper.append($("<img/>").attr("src",record[column.field]).attr("height", "80").attr("width", "55"))
                //.addClass($grid.data('grid').uiLibrary === "bootstrap" ? "glyphicon" : "ui-icon")
                // .addClass(column.icon).css({ "cursor": "pointer" }));
            }
        }
